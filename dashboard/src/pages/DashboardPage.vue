...
onMounted(async () => {
  store.init()
  // Load build filter from query param (?build=apurva)
  const buildName = route.query.build
  if (buildName) {
    await store.loadBuildRuns(buildName)
  } else {
    store.activeBuild = null
  }
  // Auto-select run from URL param
  if (props.runId && props.runId !== store.selectedRun) {
    store.selectRun(props.runId)
  }
})

// Sync URL changes to store
watch(() => props.runId, (id) => {
  if (id && id !== store.selectedRun) store.selectRun(id)
})
...
function selectRunFromSidebar(runId) {
  store.selectRun(runId)
  if (runId) {
    router.replace({ name: 'dashboard-run', params: { runId } })
  } else {
        ...
        <div class="w-72 flex-1 flex flex-col min-h-0">
          ...
          <div class="px-4 py-3 border-b border-slate-700 shrink-0 flex items-center justify-between">
            <span class="text-xs font-semibold uppercase tracking-wider text-slate-400">
              {{ store.activeBuild ? store.activeBuild : 'Tests' }}
            </span>
            <div class="flex items-center gap-2">
              <span
              ...
              <span class="text-xs font-mono text-slate-600">{{ store.sidebarRuns.length }}</span>
            </div>
          </div>
          ...
          <div class="flex-1 overflow-y-auto min-h-0">
            <button
              v-for="run in store.sidebarRuns"
              :key="run.id"
              @click="selectRunFromSidebar(run.id)"
              :class="[
            ...
            <!-- Empty state -->
            <div v-if="!store.sidebarRuns.length" class="px-4 py-6 text-xs text-slate-600 italic">
